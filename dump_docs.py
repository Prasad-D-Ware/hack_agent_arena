...
Usage:
  python dump_docs.py [output_path]      # default: assets/api_docs.json
"""
...
OUT = sys.argv[1] if len(sys.argv) > 1 else "assets/api_docs.json"


def main() -> int:
    task_id = load_task_ids("dev")[0]  # any task; we only need the sandbox's `apis`
    with AppWorld(task_id=task_id, experiment_name="dump_docs") as world:
        apps = json.loads(world.execute(
        ...
        for a in apps:
            ...
            apis_out = json.loads(world.execute(code).strip())
            artifact_apps[app] = {
                "description": a.get("description", ""),
                "api_count": len(apis_out),
                "apis": apis_out,
            }
            print(f"  {app}: {len(apis_out)} apis")

    app_count = len(artifact_apps)
    api_count = sum(v["api_count"] for v in artifact_apps.values())
    artifact = {
        "meta": {
    ...
    print(f"\nWROTE {OUT}  ->  {app_count} apps, {api_count} apis")
    return 0
